...
function getSwis()
{
    ...
    /* The 55 small model uses a different definition. */
    if (ROV.getTargetName().equals("C55")) {
        var swis = Program.fetchArray(SWI.SWI_Obj_55$fetchDesc, tabBegAddr, tabLen);
    }
    else {
...
function fxnToHandle(fxn) {
     ...
     * function address.
     */
    var fxnAddr = Program.fetchStruct(ScalarStructs.S_Fxn$fetchDesc, fxn).elem;
