...
/** ============================================================================
 *  @func   PMGR_PROC_sendTerminateEvent
 *
...
NORMAL_API
DSP_STATUS
PMGR_PROC_sendTerminateEvent(IN ProcessorId procId)
{
    DSP_STATUS status = DSP_SOK;

    TRC_1ENTER("PMGR_PROC_sendTerminateEvent", procId);

#if !defined(ONLY_PROC_COMPONENT) && (DSPLINK_TERMINATE_EVT != (-1))
    /* send terminate event to dsp */
    status = IPS_notify(procId, 0, DSPLINK_TERMINATE_EVT, 0, FALSE);
    ...
    TRC_1LEAVE("PMGR_PROC_sendTerminateEvent", status);
